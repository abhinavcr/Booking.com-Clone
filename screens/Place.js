...
const Place = () => {
  ...
  const searchedPlace = data?.filter(
  ...
  const applyFilter = (filter) => {
    ...
    switch (filter) {
      case "cost:High to Low":
        searchedPlace.map((val) =>
          val.properties.sort((a, b) => {
            a.newPrice - b.oldPrice;
          })
        );
        setSortedData(searchedPlace);
        break;
      case "cost:Low to High":
        searchedPlace.map((val) =>
          val.properties.sort((a, b) => {
            b.oldPrice - a.newPrice;
          })
        );
        setSortedData(searchedPlace);
        break;
  ...
  return (
    <View style={styles.cont}>
      <View style={styles.sortbar}>
        ...
        <Pressable style={styles.sorticon}>
          <Ionicons name="filter" size={24} color="black" />
          <Text style={styles.sorttext}>Filter</Text>
        </Pressable>
        <Pressable onPress={() =>{"Map", {place:searchedPlace}}} style={styles.sorticon}>
          <Feather name="map-pin" size={24} color="black" />
          <Text style={styles.sorttext}>Map</Text>
        </Pressable>
      </View>
      {/* {loading ? (
          ...
          .map((item) =>
            item.properties.map((property, index) => (
              <Propertycard
                ...
                childres={route.params.Children}
                adults={route.params.Adult}
                selectedDates={route.params.SelectedDate}
                property={property}
      ...
      <BottomModal
        ...
        footer={
          <ModalFooter>
            <ModalButton
              text="Apply"
              onPress={() => applyFilter(sortFilter)}
              align="center"
            ...
            <View style={{ flex: 3 }}>
              {filters.map((item, index) => (
                <Pressable
                  ...
                  onPress={() => setSortFilter(item.filter)}
                >
                  {sortFilter.includes(item.filter) ? (
                    <FontAwesome name="circle" size={20} color="green" />
                  ) : (
                    <Entypo name="circle" size={20} color="black" />
                  )}
                  <Text style={{ marginLeft: 5 }}>{item.filter}</Text>
                </Pressable>
              ))}
            </View>
          </View>
...
const styles = StyleSheet.create({
  cont: { marginTop: Platform.OS === "android" ? 25 : 10 },
  sortbar: {
    flexDirection: "row",
